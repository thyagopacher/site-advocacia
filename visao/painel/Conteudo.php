...
if (isset($_GET["codconteudo"])) {
    $resconteudo = $conexao->comando("select * from conteudo where codconteudo = '{$_GET["codconteudo"]}'");
    $conteudo = mysql_fetch_array($resconteudo);
    $action = "../../control/AtualizarConteudo.php";
} else {
    $action = "../../control/InserirConteudo.php";
}
$rescategoria = $conexao->comando("select * from categoria order by nome");
$qtdcategoria = mysql_num_rows($rescategoria);
?>
<!DOCTYPE html>
<!--
...
<html>
    <head>
        ...
        <title>Conteúdo - Painel Administrativo Advocacia</title>
    ...
    <body>
        ...
        include 'menu.php';
        ?>
        <form id="fconteudo" name="fconteudo" action="<?= $action ?>" method="post">
            <fieldset>
                <legend>Cadastro de conteúdo</legend>

                <input type="hidden" name="codcategoria" id="codcategoria" value="<?= $conteudo["codcategoria"] ?>"/>
                ...
                <p>
                    ...
                    <select name="codcategoria" id="codcategoria">
                        ...
                        if ($qtdcategoria > 0) {
                            ...
                            while ($categoria = mysql_fetch_array($rescategoria)) {
                ...
                <p>
                    <label>Texto</label>
                    <textarea cols="80" rows="10" name="texto" id="texto" class="texto" required placeholder="Digite o conteúdo aqui...">
                        <?= $conteudo["texto"] ?>
                    </textarea>
                </p>   
                <p>
                ...
                <p>
                    ...
                    <?php } else { ?>
                        <input type="submit" name="btatualizar" id="btatualizar" value="Atualizar"/>
